...
export class WalkerPhysics {
    ...
    constructor(x: number, y: number, r: number, legLength: number, legWidth: number) {
        ...
        //create the joints
        const rightHip = Constraint.create({bodyA: this.body, bodyB: this.upperRightLeg, pointA: {x: rightHipPos.x - x, y: rightHipPos.y - y}, pointB: {x: legWidth/2, y: -legLength/2}}); 
        const rightKnee = Constraint.create({bodyA: this.upperRightLeg, bodyB: this.lowerRightLeg, pointA: {x: legWidth/2, y: legLength/2}, pointB: {x: legWidth/2, y: -legLength/2}});
        const leftHip = Constraint.create({bodyA: this.body, bodyB: this.upperLeftLeg, pointA: {x: leftHipPos.x - x, y: leftHipPos.y - y}, pointB: {x: legWidth/2, y: -legLength/2}});
        const leftKnee = Constraint.create({bodyA: this.upperLeftLeg, bodyB: this.lowerLeftLeg, pointA: {x: legWidth/2, y: legLength/2}, pointB: {x: legWidth/2, y: -legLength/2}});
        ...
        //add parts to world
        World.add(getEngine().world, [this.body, this.upperRightLeg, this.upperLeftLeg, this.lowerRightLeg, this.lowerLeftLeg, rightHip, rightKnee, leftHip, leftKnee]);
    }
